...
class Co3dDatasetSingle(BaseDataset):
    def __init__(
        self,
        common_conf,
        split: str = "train",
        ...
        only_category: str = "tv",
        min_num_images: int = 24,
        len_train: int = 100000,
        len_test: int = 10000,
    ):
        """
        A single-category variant of Co3D dataset that loads only one specified category
        (e.g., "tv"). This avoids scanning all SEEN_CATEGORIES and is suitable for
        subset training.

        Args:
            common_conf: Common configuration (aug flags, debug, etc.).
            split: "train" or "test".
            CO3D_DIR: Root directory for CO3D images/depths/depth_masks.
            CO3D_ANNOTATION_DIR: Directory containing {category}_{split}.jgz annotation files.
            only_category: The single category to load (e.g., "tv").
            min_num_images: Minimum images per sequence to keep.
            len_train/len_test: Virtual length for repeated sampling.
        """
        ...
        # Always use the provided single category irrespective of debug flag
        ...
        for c in category:
            for split_name in split_name_list:
                ...
                try:
                    ...
                    logging.error(f"Failed to open gzip file {annotation_file}: {e}")
                    continue
        ...
        logging.info(f"{status}: Co3D ({only_category}) Data size: {self.sequence_list_len}")
        logging.info(f"{status}: Co3D ({only_category}) dataset length: {len(self)}")
