...
# and adapts the pipeline to call model(images) directly.
...
def parse_args():
    parser = argparse.ArgumentParser(description="VGGT Demo (Optimized)")
...
def run_VGGT(model, images, dtype, resolution=518):
    ...
    # Debug: timing + peak CUDA memory (for VGGT forward)
    ...
    print(f"[Debug] VGGT forward time: {t1 - t0:.3f}s")
    if torch.cuda.is_available():
        print(f"[Debug] VGGT peak CUDA memory: {peak_mem/1e6:.2f} MB")
...
def demo_fn(args):
    ...
    print(f"Model loaded (optimized)")
